...
function c6220.filter1(c,tp)
	local lv=c:GetLevel()
	return lv>0 and c:IsFaceup() and Duel.IsExistingTarget(c6220.filter2,tp,LOCATION_GRAVE,0,1,nil,lv)
end
function c6220.filter2(c,lv)
	return c:GetLevel()==lv 
end
function c6220.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g2=Duel.SelectTarget(tp,c6220.filter2,tp,LOCATION_GRAVE,0,1,1,nil,g1:GetFirst():GetLevel())
...
function c6220.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc2:IsRelateToEffect(e) or tc2:GetLevel()~=tc1:GetLevel()  then return end
	Duel.BreakEffect()
