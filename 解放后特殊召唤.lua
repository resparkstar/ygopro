--�\ħ�g�Υ���`��  
function c82404868.initial_effect(c)  
	--Activate  
	local e1=Effect.CreateEffect(c)  
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)  
	e1:SetType(EFFECT_TYPE_ACTIVATE)  
	e1:SetCode(EVENT_FREE_CHAIN)  
	e1:SetCost(c82404868.cost)  
	e1:SetTarget(c82404868.target)  
	e1:SetOperation(c82404868.activate)  
	c:RegisterEffect(e1)  
end  
function c82404868.cost(e,tp,eg,ep,ev,re,r,rp,chk)  
	if chk==0 then return Duel.CheckReleaseGroup(tp,nil,1,nil) end  
	local g=Duel.SelectReleaseGroup(tp,nil,1,1,nil)  
	Duel.Release(g,REASON_COST)  
end
function c82404868.filter(c,e,tp)
return c:IsCanBeSpecialSummoned(e,0,tp,false,false)  
end 
function c82404868.target(e,tp,eg,ep,ev,re,r,rp,chk)  
	if chk==0 then return Duel.CheckReleaseGroup(1-tp,nil,1,nil) and
Duel.GetLocationCount(tp,LOCATION_MZONE)>0  
		and Duel.IsExistingMatchingCard(c82404868.filter,tp, LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp) end  
Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp, LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE)  
end  
function c82404868.activate(e,tp,eg,ep,ev,re,r,rp)  
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end  
	local g=Duel.SelectReleaseGroup(1-tp,nil,1,1,nil)  
	if Duel.Release(g,REASON_RULE)~=0 then
	Duel.BreakEffect() 
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)  
	local g=Duel.SelectMatchingCard(tp,c82404868.filter,tp, LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil,e,tp) 
	Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)  
	end   
end 
	
	
